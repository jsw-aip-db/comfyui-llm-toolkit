...
def get_tasks_path() -> Path:
    """Get the path to system_prompt_tasks.json."""
    global _tasks_path
    if _tasks_path is None:
        project_root = Path(current_dir).parent
        _tasks_path = project_root / "presets" / "system_prompt_tasks.json"
    return _tasks_path
...
def get_task_names() -> List[str]:
    ...
    try:
        ...
        if not tasks_path.exists():
            _load_error = "system_prompt_tasks.json not found"
            return ["Error: " + _load_error]
        ...
        with open(tasks_path, "r", encoding="utf-8") as f:
            ...
            logger.info(f"Loaded {len(_task_names_cache)} system prompt task names")
            return _task_names_cache
    except Exception as e:
        _load_error = str(e)
        logger.error(f"Error loading system prompt task names: {e}")
        return ["Error: " + _load_error]


def load_tasks() -> Dict[str, Dict]:
    """Lazily load full system prompt task data only when needed."""
    ...
    try:
        ...
        with open(tasks_path, "r", encoding="utf-8") as f:
            ...
            logger.debug(f"Loaded {len(_tasks_cache)} system prompt tasks (full data)")
            return _tasks_cache
    except Exception as e:
        logger.error(f"Error loading system prompt tasks: {e}")
        _load_error = str(e)
        return {}
...
class SystemPromptTaskGenerator:
    ...
    def generate_prompt(self, task: str, output_as_text: bool = False, context: Optional[Dict[str, Any]] = None) -> Tuple[Dict[str, Any], str]:
        ...
        tasks = load_tasks()
        if not tasks:
            err = f"Error: Failed to load system prompt tasks."
            logger.error(err)
        ...
        if output_as_text:
            ...
            logger.info(f"SystemPromptTaskGenerator: Generated prompt for task '{task}' as text output.")
        else:
            ...
            logger.info(f"SystemPromptTaskGenerator: Generated prompt for task '{task}' as system message.")
